/*let tasks = [
            {
                ...
                status: 'in-progress',
                completed: false,
                notes: 'يجب مراجعة التصميم مع الفريق',
                userNotes: '',
                subtasks: [
                    { id: 1, name: 'إنشاء الـ Wireframes', completed: true },
                    { id: 2, name: 'اختيار الألوان', completed: false },
                    { id: 3, name: 'تصميم الأيقونات', completed: false }
                ]
            },
            {
                id: 2,
                name: 'كتابة الوثائق الفنية',
                deadline: '2025-12-24',
                status: 'overdue',
                completed: false,
                notes: 'مهمة عاجلة',
                userNotes: '',
                subtasks: [
                    { id: 1, name: 'توثيق API', completed: false },
                    { id: 2, name: 'كتابة دليل المستخدم', completed: false }
                ]
            },
            {
                id: 3,
                ...
                userNotes: '',
                subtasks: [
                    { id: 1, name: 'مراجعة Backend', completed: true },
                    { id: 2, name: 'مراجعة Frontend', completed: true }
                ]
            },
            {
                id: 4,
        ...
        function updateOverview() {
            const todayTasks = tasks.filter(t => t.deadline === '2025-12-25').length;
        ...
        function saveNotes(taskId) {
            ...
            if (task) {
                task.userNotes = notesInput.value;
                    ...
                    <div class="task-item">
                        <div class="task-content">
                            <div class="task-details" style="width: 100%;">
                                ...
                                ${task.notes ? `<div class="task-notes"><strong>ملاحظات المدير:</strong> ${task.notes}</div>` : ''}
                                ${task.userNotes ? `<div class="task-notes" style="background: #e3f2fd; border-right: 3px solid #2196F3;"><strong>ملاحظاتي:</strong> ${task.userNotes}</div>` : ''}
                                
                                <button class="subtasks-toggle" onclick="toggleNotesForm(${task.id})" style="background: #2563eb; color: white; margin-top: 10px; border-radius: 5px; padding: 5px;">
                                    <svg class="icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                    </svg>
                                    ${task.userNotes ? 'تعديل الملاحظات' : 'إضافة ملاحظات'}
                                </button>
                                ...
                                <div class="subtasks" id="notes-form-${task.id}" style="padding: 15px;">
                                    ...
                                    >${task.userNotes || ''}</textarea>
                                    <button 
                                        onclick="saveNotes(${task.id})"
                                        style="background: #4CAF50; color: white; border: none; padding: 10px 20px; border-radius: 8px; cursor: pointer; margin-top: 10px; font-weight: 600;"
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  //document.querySelector('.profile-avatar').textContent = user.username.charAt(0).toUpperCase();
